...
function NachrichtenView({sb,role,account,dbTeams=[],gruppen=[],teamFilter=null}){
  ...
  const ROLLEN_OPTS=[
    ...
    {value:"administrator",label:"Administratoren"},
  ...
  async function sendAntwort(){
    ...
    setSending(true);
    const{error}=await sb.from("nachrichten_antworten").insert({
      nachricht_id:selected.id,
      autor_id:account?.id||null,
      autor_name:account?.name||"Unbekannt",
      inhalt:antwortText.trim()
    });
    if(!error){setAntwortText("");loadAntworten(selected.id);}
    setSending(false);
  }
  ...
  async function sendNachricht(){
    ...
    setSending(true);
    const payload={
      titel:neuForm.titel.trim(),
      inhalt:neuForm.inhalt.trim(),
      typ:neuForm.typ,
      autor_id:account?.id||null,
      autor_name:account?.name||"Unbekannt",
      ...
      empfaenger_team:neuForm.empfaenger_typ==="team"?neuForm.empfaenger_team:null,
    };
    const{error}=await sb.from("nachrichten").insert(payload);
    if(!error){
      setShowNeu(false);
  ...
  useEffect(()=>{
    if(!sb) return;
    const sub=sb.channel("nachrichten-realtime")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"nachrichten_antworten"},
        payload=>{if(selected&&payload.new.nachricht_id===selected.id) loadAntworten(selected.id);})
  ...
  function getGruppe(n){
    ...
    let datum;
    if(d.toDateString()===heute.toDateString()) datum="Heute";
    else if(d.toDateString()===gestern.toDateString()) datum="Gestern";
    else if(d>woche) datum="Diese Woche";
    else datum="Älter";
  ...
  function getEmpfaengerLabel(n){
    ...
    if(n.empfaenger_typ==="gruppe"){
      const g=gruppen.find(g=>g.id===n.empfaenger_gruppe_id);
      return g?.name||"Gruppe";
    }
    return "";
  }
  ...
  function formatTime(ts){
    const d=new Date(ts);
    const heute=new Date();
    if(d.toDateString()===heute.toDateString()) return d.toLocaleTimeString("de-CH",{hour:"2-digit",minute:"2-digit"});
  ...
  function avColor(name){const i=(name||"").charCodeAt(0)%5;return{bg:AV_COLORS[i],txt:AV_TEXT[i]};}

  /* ── Thread Panel ── */
  const ThreadPanel=()=>(
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:isMobile?500:0}}>
      {/* Thread Header */}
      <div style={{padding:"12px 18px",borderBottom:"0.5px solid var(--border)",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:5,flexShrink:0}}>
            <span style={{fontSize:11,padding:"2px 8px",borderRadius:10,background:selected.typ==="broadcast"?"#E6F1FB":"#E1F5EE",color:selected.typ==="broadcast"?"#0C447C":"#085041",fontWeight:500}}>{selected.typ==="broadcast"?"Broadcast":"Diskussion"}</span>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{fontSize:12,color:"var(--sub)"}}>{selected.autor_name}</span>
          <span style={{color:"var(--sub)",fontSize:12}}>·</span>
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:10,border:"0.5px solid var(--border)",color:"var(--sub)"}}>{getEmpfaengerLabel(selected)}</span>
          <span style={{color:"var(--sub)",fontSize:12}}>·</span>
          <span style={{fontSize:12,color:"var(--sub)"}}>{formatTime(selected.erstellt_am)}</span>
        </div>
      </div>
              ...
              return(
                <div key={a.id} style={{background:"var(--surface2)",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                    ...
                    <span style={{fontSize:11,color:"var(--sub)"}}>{formatTime(a.erstellt_am)}</span>
                  </div>
      ...
      <div style={{padding:"0 20px 20px",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{fontSize:11,color:"var(--sub)",marginTop:5}}>
            {neuForm.typ==="broadcast"?"Empfänger können antworten, sehen aber nicht die Antworten der anderen":"Alle Empfänger sehen alle Antworten"}
          </div>
        </div>
        ...
        <div style={{marginBottom:14}}>
          ...
          {neuForm.empfaenger_typ==="rolle"&&(
            <select value={neuForm.empfaenger_rolle} onChange={e=>setNeuForm(f=>({...f,empfaenger_rolle:e.target.value}))}
              style={{width:"100%",padding:"8px 10px",borderRadius:9,border:"0.5px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontSize:13,fontFamily:FONT}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input value={neuForm.titel} onChange={e=>setNeuForm(f=>({...f,titel:e.target.value}))}
            placeholder="z.B. Neuer Trainingsplan" style={{width:"100%",padding:"8px 10px",borderRadius:9,border:"0.5px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontSize:13,fontFamily:FONT,boxSizing:"border-box",outline:"none"}}/>
        </div>
  ...
  const ListePanel=()=>(
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      <div style={{padding:"12px 14px",borderBottom:"0.5px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {Object.values(ungelesen).filter(v=>!v).length>0&&(
      ...
      <div style={{padding:"8px 14px",borderBottom:"0.5px solid var(--border)",display:"flex",gap:5,overflowX:"auto",scrollbarWidth:"none",flexShrink:0}}>
        {[null,"broadcast","diskussion","trainer","eltern"].map(f=>(
          <button key={f||"alle"} onClick={()=>setFilter(filter===f?null:f)}
            style={{padding:"4px 10px",fontSize:11,borderRadius:20,border:`0.5px solid ${filter===f?"transparent":"var(--border)"}`,background:filter===f?"var(--cc-accent)":"transparent",color:filter===f?"var(--cc-accent2)":"var(--sub)",cursor:"pointer",fontFamily:FONT,whiteSpace:"nowrap"}}>
            ...
            <div key={n.id} onClick={()=>{
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0}}>
                  {isUngelesen&&<div style={{width:7,height:7,borderRadius:"50%",background:"var(--cc-accent)",flexShrink:0}}/>}
                ...
                <span style={{fontSize:11,color:"var(--sub)",flexShrink:0,marginLeft:6}}>{formatTime(n.erstellt_am)}</span>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                <span style={{fontSize:11,padding:"2px 8px",borderRadius:10,border:"0.5px solid var(--border)",color:"var(--sub)"}}>{getEmpfaengerLabel(n)}</span>
  ...
  return(
    <div>
      ...
      {isMobile?(
        ...
        <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:0,border:"0.5px solid var(--border)",borderRadius:14,overflow:"hidden",minHeight:600}}>
