                    ...
                    <tr key={m.id} className={`cc-members-tr${selected.has(m.id)?" cc-members-tr-selected":""}`}
                      ...
                      {COLS.map(col=>{
                        switch(col.key){
                          ...
                          case "rollen": return <td key="rollen" className="cc-members-td">{(()=>{const portalLabel=m.role&&m.role!=="-"?(ROLLE_LABEL[m.role]||m.role):null;const kaderDeduped=m.rollen.filter(r=>r!==portalLabel);const all=[...(portalLabel?[{label:portalLabel,isT:portalLabel.toLowerCase().includes("trainer")}]:[]),...kaderDeduped.map((r,i)=>{const rawR=(m.kader_rollen_raw||[])[i]||"";const isT=TRAINER_KEYS.some(k=>rawR===k)||r.toLowerCase().includes("trainer");return{label:r,isT};})];return all.length>0?all.map((c,i)=><span key={i} className={`cc-role-chip cc-role-chip-sm${c.isT?" cc-role-chip-trainer":""}`} style={{marginRight:3}}>{c.label}</span>):(<span className="cc-members-td-sub">—</span>);})()}</td>;
