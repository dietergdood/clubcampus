/* ═══════════════════════════════════════════════════════════════
   ClubCampus — modules/portal/ModuleRechteTab.tsx
   ═══════════════════════════════════════════════════════════════ */
import { Card, InfoBox } from "../../theme.ts";
import { TI } from "../../icons.tsx";
import { BTN_COLOR as BTN, BTN_TXT, GN, BL, BK, FONT, STATUS_BG } from "../../constants.ts";
import { ZUGRIFF_ORDER, ZUGRIFF_LABELS, ZUGRIFF_COLORS, ZUGRIFF_ICONS, ALLE_MODULE, ROLLEN_MODULE_DEFAULT, MODUL_AKTIONEN, ROLES } from "./portalUtils.ts";
import type { ZugriffDefaultMap } from "./portalUtils.ts";
import type { ModuleAktiv, ModuleRechte, Sb, SetState, Zugriffstufe } from "../../types.ts";

type ZugriffStufenMap = Record<string, Record<string, Zugriffstufe>>;

/* Gruppe für die Funktionär-Ansicht (portal_gruppen bzw. Fallback) */
interface GruppeAnzeige {
  name: string;
  farbe?: string | null;
  module?: string[] | null;
}

const FALLBACK_GRUPPEN: GruppeAnzeige[]=[
...
const KAT_LABELS_LOKAL: Record<string,string>={kern:"Kern",sport:"Sport",betrieb:"Betrieb",kommunikation:"Kommunikation",verwaltung:"Verwaltung",admin:"Systemverwaltung"};
const KAT_ORDER=["kern","sport","betrieb","kommunikation","verwaltung","admin"];

interface ModuleRechteTabProps {
  supabase: Sb;
  loading: boolean;
  isMobile: boolean;
  ...
  expandedModul: string | null;
  setExpandedModul: SetState<string | null>;
  moduleViewMode: string;
  setModuleViewMode: SetState<string>;
  moduleDirty: boolean;
  setModuleDirty: SetState<boolean>;
  effRechte: Record<string, string[]>;
  getZugriff: (rolle: string, modulKey: string) => Zugriffstufe | null;
  cycleZugriff: (rolle: string, modulKey: string) => void;
  toggleModulGlobal: (key: string) => void;
  toggleModulRolle: (modulKey: string, rolle: string) => void;
  ROLLEN: string[];
  ROLLEN_LABELS: Record<string, string>;
  gruppen: GruppeAnzeige[];
  zugriffStufen: ZugriffStufenMap | null;
  setZugriffStufen: SetState<ZugriffStufenMap | null>;
  effZugriff: Record<string, ZugriffDefaultMap>;
}

export function ModuleRechteTab({supabase,loading,isMobile,mobileKachel,tab,vereinId,setSaveMsg,moduleAktiv,moduleRechte,setModuleRechte,expandedModul,setExpandedModul,moduleViewMode,setModuleViewMode,moduleDirty,setModuleDirty,effRechte,getZugriff,cycleZugriff,toggleModulGlobal,toggleModulRolle,ROLLEN,ROLLEN_LABELS,gruppen,zugriffStufen,setZugriffStufen,effZugriff}: ModuleRechteTabProps) {
  return (
            ...
            {moduleDirty&&(
              <>
                <button onClick={async()=>{
                  /* verein_id ist in modul_rechte NOT NULL — ohne fiel das
                     Speichern aller Modul-Rechte durch. */
                  if(supabase&&moduleRechte&&vereinId){
                    const rows: {modul:string;rolle:string;hat_zugriff:boolean;stufe:string;verein_id:string}[]=[];
              ...
              <table className="cc-table">
                ...
                <tbody>
                  {KAT_ORDER.map(kat=>{
                    ...
                    return([
                      <tr key={"kat-"+kat}>
                        <td colSpan={ROLLEN.length+1} style={{padding:"6px 14px 4px",fontSize:10,fontWeight:700,color:"var(--sub)",textTransform:"uppercase",letterSpacing:0.8,background:"var(--surface2)",borderTop:"1px solid var(--border)"}}>{KAT_LABELS_LOKAL[kat]}</td>
                      </tr>,
                      ...mods.map(m=>{
                          ...
                          <tr key={m.key} style={{borderTop:"0.5px solid var(--border)",opacity:isAktiv?1:0.35,background:isPflicht?"#FFFBEB":"transparent"}}>
                            <td style={{padding:"0",position:"sticky",left:0,background:isPflicht?"#FFFBEB":isExpanded?"var(--surface2)":"var(--surface)",zIndex:1}}>
                              <div onClick={()=>setExpandedModul(isExpanded?null:m.key)}
                                ...
                                <span style={{fontWeight:500,color:isPflicht?"#B45309":"var(--text)",fontSize:14}}>{m.label}</span>
                  ...
                  <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:6}}>
                    {(gruppen.length>0?gruppen:FALLBACK_GRUPPEN).map(g=>(
                      <div key={g.name} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:8,border:"0.5px solid var(--border)"}}>
                        <div style={{width:8,height:8,borderRadius:"50%",background:g.farbe||"#888",flexShrink:0}}/>
                        <span style={{fontWeight:500,fontSize:14,flex:1}}>{g.name}</span>
                        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                          {(g.module||[]).map(mk=>{
                            const mod=ALLE_MODULE.find(m=>m.key===mk);
                            return mod?<span key={mk} style={{fontSize:10,padding:"2px 7px",borderRadius:5,background:(g.farbe||"#888")+"15",color:g.farbe||"#888"}}>{mod.label}</span>:null;
                          })}
                        </div>
                      </div>
                    ))}
